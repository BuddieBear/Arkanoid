...
public class Ball extends MovableObject {
	...
	public void bounceOff(GameObject other) {
		...
		if (other instanceof Brick) {
			...
			Vector2D d_circle = rectCenter.vectorTo(circle.getCenter());

			// Switch vector to local space using rotation (Rect center = (0,0) and Axis Aligned)
			Vector2D localCircle = d_circle.rotate(-rotation);

			// Find the closest Point from Circle to Rect
			double closestX = HelperFunction.clamp(
					localCircle.getX(),
					-rectSize.getX() / 2.0,
					rectSize.getX() / 2.0);

			double closestY = HelperFunction.clamp(
					localCircle.getY(),
					-rectSize.getY() / 2.0,
					rectSize.getY() / 2.0);
			...
			Vector2D localNormal = closestPoint.vectorTo(localCircle.toPoint());

			// Rerotate back to the real World
			Vector2D worldNormal = localNormal.rotate(rotation);

			Vector2D normal = worldNormal.normalize();

			// Calculate new direction for ball
			Vector2D v_in = new Vector2D(getDx(), getDy());

			double dotProduct = v_in.dot(normal); // Tich vo huong
			...
			// Fix overlap after collision
			double distance = localNormal.getLength();
			double penetration = radius - distance;

			if (penetration > 0) {
				Vector2D pushDirection = normal.multiply(penetration + 0.1);
				setCenter(getCenterX() + pushDirection.getX(), getCenterY() + pushDirection.getY());
			}
		}
	}
