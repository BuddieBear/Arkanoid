...
public class ChestMenu implements View {
	...
	public void openChestMenu(GameSetup stage) {
		...
		// Define which types you want to exclude
		List<PowerUp.PowerUpType> excluded = List.of(
				PowerUp.PowerUpType.SHRINK_PADDLE,
				PowerUp.PowerUpType.HARDER_BRICK
		);

		// Remove excluded ones
		allBuffs.removeAll(excluded);

		// Shuffle and pick 4 random ones
		Collections.shuffle(allBuffs);
		chestChoices = allBuffs.subList(0, Math.min(4, allBuffs.size()));
